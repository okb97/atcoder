...
void solve() {
    ...
    while (!q.empty()) {
        ...
        auto iend = upper_bound(all(ss), mp(2 * uval, n));
        int epidx = distance(ss.begin(), iend);

        auto iunv = unvis.begin();
        while (iunv != unvis.end() && *iunv < epidx) {
            int vpidx = *iunv;
            int vidx = ss[vpidx].second;

            dist[vidx] = dist[uidx] + 1;
            q.push(vidx);

            iunv = unvis.erase(iunv);
        }
    }
