        ...
        rep(firstx, w) {
            rep(secondy, h) {
                rep(secondx, w) {
                    ...
                    rep(targety, h) {
                        rep(targetx, w) {
                            if (s[targety][targetx] != '.') continue;
                            int absfirst =
                                abs(firstx - targetx) + abs(firsty - targety);
                            int abssecond =
                                abs(secondx - targetx) + abs(secondy - targety);
                            if (absfirst <= d && s[firsty][firstx] == '.' &&
                                !visited[targety][targetx]) {
                                cnt++;
                                visited[targety][targetx] = true;
                            }

                            if (abssecond <= d && s[secondy][secondx] == '.' &&
                                !visited[targety][targetx]) {
                                cnt++;
                                visited[targety][targetx] = true;
                            }
