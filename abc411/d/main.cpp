...
struct Node {
    ...
    string suffix;
    Node(shared_ptr<Node> p, const string &s) : parent(p), suffix(s) {}
};
...
int main() {
    ...
    vector<shared_ptr<Node>> pc(n + 1);
    shared_ptr<Node> server = make_shared<Node>(nullptr, "");
    rrep(i, n) pc[i] = make_shared<Node>(nullptr, "");
    ...
    string ans;
    vs part;
    shared_ptr<Node> cur = server;
    while (cur) {
        part.pb(cur->suffix);
        cur = cur->parent;
    }
    repd(i, part.size()) ans += part[i];
    out(ans);
    return 0;
}